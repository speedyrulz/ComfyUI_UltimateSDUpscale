...
| `reference_strength` | FLOAT | Number Input | 1.00 | 0.0-3.0 (step 0.01) | How strongly the reference latent influences each tile. These models have no native strength for reference latents, so the latent itself is scaled towards the neutral point of the model's latent format: 1.00 uses it as is, lower values weaken it towards a blank reference, and values above 1.00 exaggerate it and can push the model out of distribution. Also applies to reference latents supplied through the conditioning. |
